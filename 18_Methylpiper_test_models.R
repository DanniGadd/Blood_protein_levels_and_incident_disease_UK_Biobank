...
# Index how many metrics available for each trait
...
for(i in 1:length(diseases)){
  tryCatch({
    ...
    write.csv(trait, paste0('Results/Cox/Score_processing/Weights_sum/', name, '_V2.csv'), row.names = F)
  }, error = function(e) cat("skipped"))
}
...
for(i in 1:length(diseases)){
  tryCatch({
    name <- diseases[i]
    print(name)
    path <- '01_paper/Results/Cox/Score_processing/Weights_sum/'

    file <- read.csv(paste0(path, name, '_V2.csv'))
    print(length(which(file$V1 == 1)))
  }, error = function(e) cat("skipped"))
}

# Now go back and adjust median model selection accordingly

diseases <- c('AL_FO', 'ALS_FO', 'Breast_FO', 'CYS_FO', 'Colorectal_FO', 'COPD_FO', 'ENDO_FO','DEATH', 'Diab_FO', 'GYN_FO',
              'IBD_FO', 'IHD_FO', 'LIV_FO', 'LUNG_FO', 'PD_FO', 'Prostate_FO', 'RA_FO', 'ST_FO', 'VD_FO')

extra <- c(0,8,1,0,1,0,0,0,0,3,0,0,0,0,0,0,0,0,0)

# Calculate median difference for models that do have all 50 metrics
list_chosen <- list()
...
write.csv(res, 'Results/Cox/Score_processing/models_chosen_accounted_features.csv', row.names = F)
...
write.csv(bind1, 'Score_processing/features_selected_table_weights.csv', row.names = F)

write.csv(table1, 'Cox/Score_processing/features_summary_table.csv', row.names = F)
...
dat <- read.csv("Cox/Score_processing/features_summary_table.csv")
...
pdf("Cox/Score_processing/Plot_num_features.pdf", width = 9, height = 4)
