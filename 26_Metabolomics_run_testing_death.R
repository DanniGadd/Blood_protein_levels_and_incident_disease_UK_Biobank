...
location_out1 <- 'Cox/metab_analysis/Death/prot/'
location_out2 <- 'Cox/metab_analysis/Death/metab/'
location_out3 <- 'Cox/metab_analysis/Death/joint/'
...
test <- function(mprModel, testData, testTarget, res_list, seed_iteration, iter, name_dis, output, thr){
  tryCatch({
    ...
    saveRDS(testTarget, paste0(output, '/files/testResults_', iter, '_rerun_3cv.rds'))
    
    # save metrics table performances
    saveRDS(metricsTable, paste0(output, '/files/metricsTable_', iter, '_rerun_3cv.rds'))
...
### Run testing proteins
    ...
    x_test <- readRDS(paste0(location_out1, 'files/x_test_18', '.rds'))
    y_test <- readRDS(paste0(location_out1, 'files/y_test_18', '.rds'))
...
x_test <- readRDS(paste0(location_out2, '/files/x_test_13', '.rds'))
y_test <- readRDS(paste0(location_out2, '/files/y_test_13', '.rds'))
model_load <- readRDS(paste0(location_out2, '/files/model_', it, '_rerun.rds'))

mod <- test(model_load, x_test, y_test, list, seed, iteration, name, location_out2, thr_input)

result <- mod
write.csv(result, paste0(location_out2, '/metricsTables_joint_rerun_3cv.csv'))
...
x_test <- readRDS(paste0(location_out3, '/files/x_test_18', '.rds'))
y_test <- readRDS(paste0(location_out3, '/files/y_test_18', '.rds'))
...
prot <- readRDS("Death/prot/files/testResults_13.rds")
metab <- readRDS("Death/metab/files/testResults_13.rds")
joint <- readRDS("Death/joint/files/testResults_13.rds")
...
# Evaluate scores with relevant covariates in same test population
testTarget <- y_sub
riskFactorsOnlyCoxPH <- coxph(Surv(time_to_event, Event) ~ Age_assessment + as.factor(Sex), testTarget)
...
write.csv(metricsTable, 'Results/Cox/metab_analysis/metab_comp_death.csv', row.names = T)
...
pdf('Cox/metab_analysis/metab_rocs_death.pdf', width = 13.5, height = 6)
rocs_df %>%
...
metab <- read.csv("Cox/metab_analysis/Death/metab/files/weights_13.csv")
prot <- read.csv("Cox/metab_analysis/Death/prot/files/weights_13.csv")
joint <- read.csv("Cox/metab_analysis/Death/joint/files/weights_13.csv")
...
write.csv(join, 'Cox/metab_analysis/metab_weights_death.csv', row.names = F)

# Join both weights tables together
join$Outcome <- 'Death'
join2 <- read.csv('Cox/metab_analysis/metab_weights_diab.csv')
join2$Outcome <- 'Type 2 Diabetes'
bind <- rbind(join, join2)
write.csv(bind, 'Cox/metab_analysis/metab_weights_bound_suppl_together.csv', row.names = F)
