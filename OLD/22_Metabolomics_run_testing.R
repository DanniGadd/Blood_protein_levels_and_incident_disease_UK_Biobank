...
location_out1 <- '/path_to_file.../prot/'
location_out2 <- '/path_to_file.../metab/'
location_out3 <- '/path_to_file.../joint/'
...
### Run testing proteins
    ...
    x_test <- readRDS(paste0(location_out1, name, '/files/x_test_', i, '.rds'))
    y_test <- readRDS(paste0(location_out1, name, '/files/y_test_', i, '.rds'))
    model_load <- readRDS(paste0(location_out1, name, '/files/model_', it, '.rds'))
    
    
    
    mod <- test(model_load, x_test, y_test, list, seed, iteration, name, location_out1, thr_input)
result <- mod
write.csv(result, paste0(location_out1, name, '/metricsTables_joint.csv'))
...
x_test <- readRDS(paste0(location_out2, name, '/files/x_test_', i, '.rds'))
y_test <- readRDS(paste0(location_out2, name, '/files/y_test_', i, '.rds'))
model_load <- readRDS(paste0(location_out2, name, '/files/model_', it, '.rds'))
mod <- test(model_load, x_test, y_test, list, seed, iteration, name, location_out2, thr_input)
result <- mod
write.csv(result, paste0(location_out2, name, '/metricsTables_joint.csv'))
...
x_test <- readRDS(paste0(location_out3, name, '/files/x_test_', i, '.rds'))
y_test <- readRDS(paste0(location_out3, name, '/files/y_test_', i, '.rds'))
model_load <- readRDS(paste0(location_out3, name, '/files/model_', it, '.rds'))
mod <- test(model_load, x_test, y_test, list, seed, iteration, name, location_out3, thr_input)
result <- mod
write.csv(result, paste0(location_out3, name, '/metricsTables_joint.csv'))

###############################################################################################

# Load the three scores
prot <- readRDS("/path_to_files.../Diab_FO/files/testResults_18.rds")
metab <- readRDS("/path_to_files.../Diab_FO/files/testResults_18.rds")
joint <- readRDS("/path_to_files.../Diab_FO/files/testResults_18.rds")

# > identical(prot$SampleID, metab$SampleID)
# [1] TRUE
# > identical(prot$SampleID, joint$SampleID)
# [1] TRUE
...
merge <- read.csv('/path_to_files.../imputed_covs_transformed.csv')
...
row.names(metricsTable) <- c('MetabScore',
                             'ProteinScore',
                             'MetabProteinScore',
...
write.csv(metricsTable, '/path_to_files.../metab_diabetes_comparison_ROC_p_PGS.csv', row.names = T)
...
cbPalette <- c("red", "darkorchid", "blue", "tan1", 'grey',
               "black")

pdf('/path_to_files.../DIAB_metab_rocs_diabetes_metab.pdf', width = 13.5, height = 6)
rocs_df %>%
...
metab <- read.csv("/path_to_files.../metab/Diab_FO/files/weights_18.csv")

prot <- read.csv("/path_to_files.../prot/Diab_FO/files/weights_18.csv")

joint <- read.csv("/path_to_files.../joint/Diab_FO/files/weights_18.csv")
...
write.csv(join, '/path_to_files.../joint_metab_prot_weights.csv', row.names = F)
